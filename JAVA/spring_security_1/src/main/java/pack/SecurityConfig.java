...
@Configuration
@EnableWebSecurity
public class SecurityConfig {  // 애플리케이션의 보안 구성을 설정(보안 설정, 인증, 폼로긴 ...)
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
		http
			...
			.formLogin(flog ->   
				...
				//Security가 자동으로 로그인(/login <== endpoint), 로그인 실패(/login?error)를 생성한다.
				flog
					.loginPage("/login")   // 사용자 정의 페이지를 지정
					.defaultSuccessUrl("/", true)   // 인증에 성공한 후 redirect 될 기본 URL 지정
					.permitAll()
			)
			.logout(logout ->   // 로그아웃 기능을 설정 (필요할 때만)
				logout
	...
	// login.html에서 user, 제공된 password 대신 내가 입력한 id, password를 사용한다면...
	@Bean
	public UserDetailsService userDetailsService() {
		...
		// InMemoryUserDetailsManager : 영구적 저장소 사용 X. 재시작하면 정보 사라짐 - 테스트용
